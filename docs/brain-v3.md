...
- **模式**：她在设置里定义模式（名字 / 什么时候用 / prompt，`profile.modes`），回复模型只有一个（默认 4.2），模式只在人设后面加它的 prompt。reflect 在每轮结束时排好接下来的模式安排 `mode_plan`（几小时后起用哪个，写进 `qr_mode_log`，未来的行每轮整体替换）：哄她去学了、她不回了 → 下次是戏。休息带 until，到点回到现实并主动叫她。当天还没决定时：工作日 8–20 点是现实，其余是戏。
