...
- **回复**：一个模型（默认 4.2）；当前模式决定在人设后面加哪段 prompt。prompt = 人设 +【我们磨合出来的】（记忆里的磨合段）+【我此刻】（reflect 的 thought）+ 时间和今日时间事实 + 最近 20 条。回复前不调用任何模型。
- **模式**：她在设置里定义模式（名字 / 什么时候用 / prompt，`profile.modes`），回复模型只有一个（默认 4.2），模式只在人设后面加它的 prompt。reflect 在每轮结束时排好接下来的模式安排 `mode_plan`（几小时后起用哪个，写进 `qr_mode_log`，未来的行每轮整体替换）：哄她去学了、她不回了 → 下次是戏。休息结束叫她回去学，由 reflect 在 reaches 里自己定一件。当天还没决定时：工作日 8–20 点是现实，其余是戏。
...
- **主动找她**：计划存在 `qr_reach_plans`，同时可以挂好几条（`done_at` 为空 = 待发）。reflect 每轮看到全部待发计划，给出完整的 `reaches` 列表，只替换 reflect / reach 自己定的那些（包括休息结束叫她回来的那件）；Rosie 手动加的（`set_by=rosie`）不动。wake cron 到点把所有到期的计划一起交给 reach，发完标 done，其余继续挂着；reach 给的 next_reach 追加成新的一条。`qr_reach` 只剩开关和 retry。
