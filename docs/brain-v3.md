# 清然 brain v4（2026-09-25，覆盖下面 v3 的对应部分）

- 回复：人设 + 【我此刻】（只有心思刚想出的新念头，没有就整块删掉）+ 时间 + history。回复不再读记忆和故事线。
- 心思（reflect）：每轮回复后跑。读人设、故事线（设置 → 人设 → 故事线）、记忆、最近 12 条念头（清空聊天之后的）、最近 20 条对话。输出 `{thought, scene, next_reach}`。thought 可以为空，为空就不注入。念头存在 `qr_inner.now_text`，历史就是 `qr_inner_log`。desire / read_her / feel / choice / plans / glow 不再写。
- 记忆（editor）：每 60 轮或隔了一次会话，旧记忆 + 新对话 → 整篇重写（`{body}`），三段：我看出来的你 / 我们之间 / 还悬着的事。没有 ops 了。只有心思读它。
