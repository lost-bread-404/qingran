...
高频 route（`voice` / `reflect` / `archive`）**只存引用，不存原文**。发给模型的 prompt 都是数据库里已有原料拼出来的；查看时用同一份拼装函数重建。

## 表

| 表 | 用途 | 保留 |
|---|---|---|
| `brain_log` | 每一次模型调用：route / 模型 / tokens / 费用 / `refs` / `output_ref` | 数值和引用永久；`output_text` / `input_*` 默认 7 天（`QR_LOG_TEXT_DAYS`）后裁到 `raw` 前 300 字 |
...
| `brain_log_raw` | 可选完整输入（`QR_LOG_RAW_HOURS`，默认关） | 按时数删除 |
...
| 低频（dusk / portrait / assign / backfill / synth / report / ask / judge） | — | 继续存完整 `input_system` / `input_user` / `output_text` |
...
调试窗口：`QR_LOG_RAW_HOURS>0` 时高频 route 额外把完整输入写入 `brain_log_raw`。
...
旧高频日志的完整输入在首次 cron 分批清空（每批 500 行），然后 `VACUUM`（Neon 上若不可用则跳过）。
