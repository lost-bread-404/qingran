...
- 听力引擎：设置 → 听力。调试开着时每一句都存 clip。定向录制：`/record`（只存 WAV，不调 LLM / STT）。标注页：`/lab`。
- **xAI STT 官方不支持中文。** `/v1/stt` 的 `language` 格式化语言列表是 en / fr / de / ja 等，没有 zh。代码不再传未文档化的 `prompt` 字段，显式使用 `model=grok-voice-transcribe-2.0`，并保留 `filler_words=true`、`vad_threshold=0`。
