...
export function FlagReply({ open, triggerText, replyText, busy, error, rating = "down", onClose, onSave }: Props) {
    ...
    <div className="fixed inset-0 z-50 flex flex-col justify-end bg-bg/80">
      ...
      <div className="rounded-t-2xl bg-bg px-4 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-3 shadow-lamp">
        <div className="mb-3 flex items-center justify-between">
          <p className="font-display text-lg">{rating === "up" ? "这条回复好" : "这条回复不好"}</p>
          <button
            type="button"
            aria-label="关掉"
            onClick={onClose}
