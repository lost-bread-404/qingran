...
test("A/B/C system prompts stay out of the role and off the calendar", () => {
  assert.match(PROMPT_A_SYSTEM, /不扮演/);
  assert.match(PROMPT_A_SYSTEM, /跨夜/);
  assert.doesNotMatch(PROMPT_A_SYSTEM, /不要分析人格/);
  assert.match(PROMPT_C_SYSTEM, /也不改清然的人设/);
  assert.match(PROMPT_C_SYSTEM, /不要用诊断标签/);
});
