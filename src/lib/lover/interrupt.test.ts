...
test("room messages persist the interrupted flag", () => {
  const markup = readFileSync(new URL("./message-markup.ts", import.meta.url), "utf8");
  const room = readFileSync(new URL("./room.ts", import.meta.url), "utf8");
  assert.match(markup, /if \(msg\.interrupted\) text = `⟦断⟧\$\{text\}`/);
  assert.match(markup, /text\.startsWith\("⟦断⟧"\)/);
  assert.match(room, /interrupted: decoded\.interrupted \|\| undefined/);
});
