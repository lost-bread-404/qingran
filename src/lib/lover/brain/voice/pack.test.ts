...
test("voice messages keep charter first and tail last before user", () => {
  const msgs = buildVoiceMessages({
    ...
    selfSummary: "我在医学院",
    bondSummary: "叫她小猫",
    portrait: [{ id: "p", topic: "安慰", body: "别讲道理", status: "active", kind: "trait", evidenceIds: [], lastSeen: 0, lastSupportedAt: 0, supportCount: 1, updatedAt: 0 }],
    history: [
...
test("care checkin line is gone", () => {
  ...
  assert.doesNotMatch(on, /今天过得怎么样/);
});
...
test("voice inject switches omit memories, longterm, and history independently", () => {
  const base = {
    ...
    selfSummary: "我在医学院",
    bondSummary: "叫她小猫",
    portrait: [{ id: "p", topic: "安慰", body: "别讲道理", status: "active" as const, kind: "trait" as const, evidenceIds: [], lastSeen: 0, lastSupportedAt: 0, supportCount: 1, updatedAt: 0 }],
    history: [
...
test("reply slots keep stored wording; charter, history, and this turn stay", () => {
  ...
  assert.match(longterm, /清然在医学院/);
  assert.match(longterm, /叫她小猫/);
  assert.match(longterm, /Rosie 今天日程很满/);
  assert.match(tail, /清然承诺陪 Rosie 写完/);
  assert.match(tail, /她其实一直在怕/);
