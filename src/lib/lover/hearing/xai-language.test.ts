...
import { STT_KEYTERMS, lockSttKeyterms, xaiVadThreshold } from "./config.ts";
...
test("STT_KEYTERMS is the slim configurable list in hearing/config.ts", () => {
  ...
  assert.deepEqual(lockSttKeyterms(undefined), [...STT_KEYTERMS]);
  assert.deepEqual(lockSttKeyterms([" 林泽 ", "林泽", "", "嗯"]), ["林泽", "嗯"]);
  assert.equal(lockSttKeyterms([]).length, 0);
