...
test("settings hearing tab is sensitivity, the live request, and labeling", () => {
  ...
  assert.match(src, />上下文长度</);
  assert.match(src, /brainSyncHistoryWindow/);
  assert.match(src, /parseVoiceInjectLine/);
  assert.doesNotMatch(src, /检索照常跑/);
  assert.match(types, /injectMemories/);
  assert.match(types, /injectLongterm/);
  assert.match(types, /historyWindow/);
});
...
test("settings prompts tab edits catalog steps and log expand shows assembled prompt", () => {
  ...
  assert.match(src, /PromptStepEditor/);
  assert.match(src, /brainRollbackPrompt/);
  assert.match(editor, /查看当前内容/);
  assert.match(editor, /预览实际发送内容/);
  assert.match(editor, /加一条消息/);
  assert.match(editor, /回退到/);
  assert.match(editor, /恢复默认/);
  assert.match(editor, /brainPreviewPrompt/);
  assert.match(api, /brainPreviewPrompt/);
  assert.match(api, /brainRollbackPrompt/);
...
test("voice room keeps the meter and writes final_text back; the labeled count lives in settings", () => {
  ...
  assert.match(src, /unselectedReplyIds/);
  assert.match(src, /keepReplies/);
...
test("lab page is score card, worst 20, labels, homophones, and hash export", () => {
  ...
  assert.match(src, /formatToneReadingLine/);
  assert.match(src, /senseLine/);
