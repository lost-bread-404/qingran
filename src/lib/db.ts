...
const onVercel = Boolean(typeof process !== "undefined" && process.env.VERCEL);
...
async function applyNeonMigrations(pool: import("pg").Pool) {
  // Runtime apply covers the case where `npm run db:migrate` skipped at Vercel
  // build because DATABASE_URL was not in the build environment. Pending files
  // are tracked in `_migrations`, so this is idempotent. Use a transaction
  // advisory lock — Neon pooled endpoints do not keep session locks.
